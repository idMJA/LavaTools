...
const playerVariantDetails: VariantDetail[] = [
	new VariantDetail(
		PlayerVariant.IAS,
		/^player_ias\.vflset\/([a-zA-Z_]+)\/base\.js$/,
		(region) => `player_ias.vflset/${region}/base.js`,
	),
	new VariantDetail(
		PlayerVariant.IAS_TCC,
		/^player_ias_tcc\.vflset\/([a-zA-Z_]+)\/base\.js$/,
		(region) => `player_ias_tcc.vflset/${region}/base.js`,
	),
	new VariantDetail(
		PlayerVariant.IAS_TCE,
		/^player_ias_tce\.vflset\/([a-zA-Z_]+)\/base\.js$/,
		(region) => `player_ias_tce.vflset/${region}/base.js`,
	),
	new VariantDetail(
		PlayerVariant.ES5,
		/^player_es5\.vflset\/([a-zA-Z_]+)\/base\.js$/,
		(region) => `player_es5.vflset/${region}/base.js`,
	),
	new VariantDetail(
		PlayerVariant.ES6,
	...
	new VariantDetail(
		PlayerVariant.TV_ES6,
		/^tv-player-es6\.vflset\/tv-player-es6\.js$/,
		() => "tv-player-es6.vflset/tv-player-es6.js",
	),
	new VariantDetail(
		PlayerVariant.EMBED,
...
export class PlayerScript {
	...
	toUrl(): string {
		const detail = playerVariantDetails.find(
			(item) => item.variant === this.variant,
		);
		if (!detail) {
...
export function getPlayerScript(playerUrl: string): PlayerScript {
	...
	if (overridePlayerVariant) {
		const variant =
			PlayerVariant[overridePlayerVariant as keyof typeof PlayerVariant];
		if (variant) {
